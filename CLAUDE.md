...
**Frontend `api()` wrapper** ([frontend/src/lib/api.ts](frontend/src/lib/api.ts)): auto-refreshes on 401 with a single-flight lock, attaches CSRF, and **only retries `GET`/`HEAD` on network errors** — never mutating verbs (would risk duplicate charges). `ApiError.code` exposes the server's stable error code — switch on `.code`, not on `.message`.
...
**Cron strategy.** No `setInterval` loops — Next.js / Vercel doesn't keep long-lived processes. Background work runs as **Vercel Cron** routes under `app/api/cron/<name>/route.ts`, each gated by `Authorization: Bearer ${CRON_SECRET}`. Design targets (the frequency each job is meant to run at): `outbox-drain` (1m), `email-queue-drain` (1m), `verification-cleanup` (hourly), `order-expiration` (5m), `webhook-log-purge` (daily), `email-job-purge` (daily — purges SENT EmailJob rows older than `EMAIL_JOB_RETENTION_DAYS`, default 30). **Current production schedule** in [frontend/vercel.json](frontend/vercel.json) runs `outbox-drain`/`email-queue-drain`/`verification-cleanup`/`order-expiration` once daily instead, because the Vercel **Hobby (free) plan caps cron jobs at once per day** — revert those four to their design targets above once the project is on a paid Vercel plan. Multi-instance coordination still uses [frontend/src/lib/server/leader-lease.ts](frontend/src/lib/server/leader-lease.ts) Redis leases where two crons could collide. The Bictorys charge `CircuitBreaker` is still in-memory single-instance — replace with a Redis-backed variant for multi-pod prod (documented limitation).
...
- [examples/frontend-pages/](examples/frontend-pages/) — reference pages (login/signup/verify-email/forgot-reset-password/dashboard/payment-success+failure/auth-error/admin/*). They are NOT imported anywhere — they live as Tailwind references to copy or rebuild.
