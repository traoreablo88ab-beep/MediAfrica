// PATCH /api/depot/alertes/[id] — mark an alert as vue/résolue and/or attach
// a resolution note (.planning/prd-depot-medicaments.md § 5.3 "Action :
...
export async function PATCH(
  ...
  return withRequestContext(ctx, async () => {
    ...
    const existing = await prisma.depotAlerte.findFirst({
    ...
    const d = parsed.data;
    const updated = await prisma.depotAlerte.update({
      where: { id },
      data: {
