...
function makePatch(body: unknown, opts: { csrf?: 'match' | 'missing' } = {}): NextRequest {
  ...
  return new NextRequest('http://test/api/guichet/alertes/al-1', {
    method: 'PATCH',
    headers,
...
beforeEach(() => {
  ...
  prismaMock.guichetAlerte.findFirst.mockResolvedValue({ id: 'al-1' } as never);
});

describe('PATCH /api/guichet/alertes/[id]', () => {
  it('missing x-csrf-token header → 403; no Prisma calls', async () => {
    const res = await callPatch({ vue: true }, 'al-1', { csrf: 'missing' });
    expect(res.status).toBe(403);
    expect(prismaMock.guichetAlerte.update).not.toHaveBeenCalled();
  });
  ...
  it('not found in this org → 404', async () => {
    prismaMock.guichetAlerte.findFirst.mockResolvedValue(null);
  ...
  it('marks an alert as vue', async () => {
    prismaMock.guichetAlerte.update.mockResolvedValue(alerteRow({ vue: true }) as never);
    const res = await callPatch({ vue: true });
    expect(res.status).toBe(200);
    const args = prismaMock.guichetAlerte.update.mock.calls[0]?.[0] as {
  ...
  it('marking resolue=true implicitly sets vue=true when vue is not sent', async () => {
    prismaMock.guichetAlerte.update.mockResolvedValue(
  ...
  it('attaches a resolutionNote', async () => {
    prismaMock.guichetAlerte.update.mockResolvedValue(
      alerteRow({ resolutionNote: 'Vérifié avec le guichetier, RAS.' }) as never,
    );
    await callPatch({ resolutionNote: 'Vérifié avec le guichetier, RAS.' });
    const args = prismaMock.guichetAlerte.update.mock.calls[0]?.[0] as {
      data: Record<string, unknown>;
    };
    expect(args.data).toEqual({ resolutionNote: 'Vérifié avec le guichetier, RAS.' });
  });

  it('scopes the lookup to the caller org (cross-org id → 404, no leak)', async () => {
    prismaMock.guichetAlerte.findFirst.mockResolvedValue(null);
    await callPatch({ vue: true }, 'al-other-org');
    const args = prismaMock.guichetAlerte.findFirst.mock.calls[0]?.[0] as {
