...
function ctxWith(
  ...
  return { params: Promise.resolve({ id, visiteId }) };
}

function makeDelete(): NextRequest {
  return new NextRequest('http://test/api/nutritions/n-1/visites/v-1', {
    method: 'DELETE',
...
describe('DELETE /api/nutritions/[id]/visites/[visiteId]', () => {
  it('requireAuth bail → 401, no Prisma calls', async () => {
    mockRequireOrgMember.mockResolvedValueOnce(
      NextResponse.json({ error: 'Missing token' }, { status: 401 }),
    );
    const res = await DELETE(makeDelete(), ctxWith('n-1', 'v-1'));
    expect(res.status).toBe(401);
    expect(prismaMock.nutritionVisiteSuivi.delete).not.toHaveBeenCalled();
  });

  it('unknown visite → 404 VISITE_NOT_FOUND', async () => {
  ...
  it('register month closed → 409 REGISTER_CLOSED, checked against the parent episode date', async () => {
    prismaMock.nutritionVisiteSuivi.findFirst.mockResolvedValue({
      id: 'v-1',
      nutritionId: 'n-1',
      nutrition: { date: new Date('2025-12-20T09:00:00Z'), type: 'URENAM' },
    } as never);
  ...
  it('URENI parent → closure checked against nutrition-ureni, not urenam', async () => {
    prismaMock.nutritionVisiteSuivi.findFirst.mockResolvedValue({
      id: 'v-1',
      nutritionId: 'n-1',
      nutrition: { date: new Date('2025-12-20T09:00:00Z'), type: 'URENI' },
    } as never);
    prismaMock.registerClosure.findUnique.mockResolvedValue({ id: 'rc-1' } as never);
    const res = await DELETE(makeDelete(), ctxWith('n-1', 'v-1'));
    expect(res.status).toBe(409);
  ...
  it('happy path: deletes and returns ok', async () => {
    prismaMock.nutritionVisiteSuivi.findFirst.mockResolvedValue({
      id: 'v-1',
      nutritionId: 'n-1',
      nutrition: { date: new Date('2026-01-12T09:00:00Z'), type: 'URENAM' },
    } as never);
    const res = await DELETE(makeDelete(), ctxWith('n-1', 'v-1'));
    expect(res.status).toBe(200);
    expect(prismaMock.nutritionVisiteSuivi.delete).toHaveBeenCalledWith({
      where: { id: 'v-1' },
    });
  });
});
