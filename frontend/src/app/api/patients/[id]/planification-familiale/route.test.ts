  ...
  describe('Idempotency-Key (offline-queue replay)', () => {
    it('no header → unchanged behavior, no idempotency fields on create', async () => {
      ...
      prismaMock.planificationFamiliale.create.mockResolvedValue({
        ...
        patientId: 'pt-1',
        date: new Date('2026-01-12T09:00:00Z'),
        typeVisite: VALID_BODY.typeVisite,
        methodeChoisie: VALID_BODY.methodeChoisie,
      } as never);

      const res = await POST(makePost(VALID_BODY), ctxWith('pt-1'));

      expect(res.status).toBe(201);
      expect(prismaMock.planificationFamiliale.findUnique).not.toHaveBeenCalled();
      const createArg = prismaMock.planificationFamiliale.create.mock.calls[0]?.[0]?.data as Record<
        string,
        unknown
      >;
    ...
    it('header + no existing row → creates and stores key/hash', async () => {
      prismaMock.patient.findFirst.mockResolvedValue({ id: 'pt-1' } as never);
      prismaMock.planificationFamiliale.findUnique.mockResolvedValue(null);
      prismaMock.planificationFamiliale.create.mockResolvedValue({
        id: 'pf-1',
        patientId: 'pt-1',
        date: new Date('2026-01-12T09:00:00Z'),
      ...
      const res = await POST(
        makePost(VALID_BODY, { idempotencyKey: 'idem-key-1' }),
        ctxWith('pt-1'),
      );
    ...
    it('header + matching replay → 200, no duplicate row created', async () => {
      ...
      const bodyHash = createHash('sha256')
        ...
        .digest('hex');
      prismaMock.planificationFamiliale.findUnique.mockResolvedValue({
        id: 'pf-existing',
        patientId: 'pt-1',
        date: new Date('2026-01-12T09:00:00Z'),
        typeVisite: VALID_BODY.typeVisite,
      ...
      const body = await res.json();
      expect(body.id).toBe('pf-existing');
      expect(prismaMock.planificationFamiliale.create).not.toHaveBeenCalled();
    });
    ...
    it('header + mismatched body → 422 IDEMPOTENCY_KEY_BODY_MISMATCH', async () => {
      ...
      prismaMock.planificationFamiliale.findUnique.mockResolvedValue({
        ...
        patientId: 'pt-1',
        date: new Date('2026-01-12T09:00:00Z'),
        typeVisite: 'Cliente continuante',
        methodeChoisie: 'DIU',
