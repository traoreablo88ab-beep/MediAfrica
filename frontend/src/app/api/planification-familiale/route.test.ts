...
function pfRow(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    id: 'pf-1',
    patientId: 'pt-1',
    providerId: 'user-1',
    ...
    methodeChoisie: 'Pilule COC',
    methodePrecedente: null,
    parite: null,
    gestite: null,
    tensionArterielle: null,
    poidsKg: null,
    counselingDonne: true,
    effetsSecondairesRapportes: null,
    quantiteRemise: '3 plaquettes',
    prochainRdv: null,
    observations: null,
    ...
    patient: {
      ...
      dateNaissance: new Date('1998-03-12T00:00:00Z'),
      sexe: 'F',
...
describe('GET /api/planification-familiale', () => {
  it('returns 401 when requireAuth bails', async () => {
    mockRequireOrgMember.mockResolvedValueOnce(
      NextResponse.json({ error: 'Missing token' }, { status: 401 }),
    );
    const res = await GET(makeGet('http://test/api/planification-familiale'));
    expect(res.status).toBe(401);
  });

  it('empty result → { items: [], nextCursor: null }', async () => {
    prismaMock.planificationFamiliale.findMany.mockResolvedValue([] as never);
    const res = await GET(makeGet('http://test/api/planification-familiale'));
  ...
  it('serializes rows with patient + providerName', async () => {
    prismaMock.planificationFamiliale.findMany.mockResolvedValue([pfRow()] as never);
    const res = await GET(makeGet('http://test/api/planification-familiale'));
    ...
    expect(body.items[0].patient).toEqual({
      ...
      prenom: 'Fatoumata',
      dossierNumber: 'P-20260001',
      dateNaissance: '1998-03-12T00:00:00.000Z',
      sexe: 'F',
      communeResidence: 'Commune V, Bamako',
    });
    expect(body.items[0].providerName).toBe('Amadou Diallo');
    expect(body.items[0].methodeChoisie).toBe('Pilule COC');
  });
  ...
  it('defaults the date filter to the start of today', async () => {
    prismaMock.planificationFamiliale.findMany.mockResolvedValue([] as never);
    await GET(makeGet('http://test/api/planification-familiale'));
    const args = prismaMock.planificationFamiliale.findMany.mock.calls[0]?.[0];
  ...
  it('?dateFrom=&dateTo= filters a range', async () => {
    prismaMock.planificationFamiliale.findMany.mockResolvedValue([] as never);
    await GET(
      makeGet('http://test/api/planification-familiale?dateFrom=2026-01-01&dateTo=2026-01-31'),
    );
    const args = prismaMock.planificationFamiliale.findMany.mock.calls[0]?.[0];
    const dateFilter = args?.where?.date as { gte?: Date; lt?: Date };
  ...
  it('?q= searches the related patient nom/prenom/dossierNumber (insensitive)', async () => {
    prismaMock.planificationFamiliale.findMany.mockResolvedValue([] as never);
    await GET(makeGet('http://test/api/planification-familiale?q=Traor%C3%A9'));
    const args = prismaMock.planificationFamiliale.findMany.mock.calls[0]?.[0];
