...
function makePost(body: unknown, opts: { csrf?: 'match' | 'missing' } = {}): NextRequest {
  ...
  return new NextRequest('http://test/api/registres/hospitalisation/close', {
    method: 'POST',
    headers,
...
describe('POST /api/registres/hospitalisation/close', () => {
  ...
  it('defaults to the current month when no month is given', async () => {
    ...
    expect(createArg?.data?.registerType).toBe('hospitalisation');
