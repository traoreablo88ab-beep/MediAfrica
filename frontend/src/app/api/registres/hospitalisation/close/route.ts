// POST /api/registres/hospitalisation/close — manually close a month's
// hospitalisation register (a single register covering every service).
// Any authenticated staff member can close (no ADMIN gate — matches the
// rest of Patients/Consultations, not back-office). Once closed, POST
// /api/patients/[id]/hospitalisation refuses new admissions AND
// PATCH/DELETE /api/hospitalisations/[id] refuses updates for admissions
// whose dateHeureEntree falls in that month (see
