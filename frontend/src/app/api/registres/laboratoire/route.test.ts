...
function makePut(body: unknown, opts: { csrf?: 'match' | 'missing' } = {}): NextRequest {
  ...
  return new NextRequest('http://test/api/registres/laboratoire', {
    method: 'PUT',
    headers,
...
describe('GET /api/registres/laboratoire', () => {
  it('returns 401 when requireAuth bails', async () => {
    mockRequireOrgMember.mockResolvedValueOnce(
      NextResponse.json({ error: 'Missing token' }, { status: 401 }),
    );
    const res = await GET(makeGet('http://test/api/registres/laboratoire'));
    expect(res.status).toBe(401);
  });

  it('no record for the month → all counters null', async () => {
    prismaMock.laboratoireRapport.findUnique.mockResolvedValue(null);
    const res = await GET(makeGet('http://test/api/registres/laboratoire?month=2026-01'));
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.month).toBe('2026-01');
    expect(body.nfsTotal).toBeNull();
    expect(body.vihPositif).toBeNull();
    expect(body.anesthesieGenerale).toBeNull();
  });

  it('defaults to the current month when no ?month= is given', async () => {
    prismaMock.laboratoireRapport.findUnique.mockResolvedValue(null);
    await GET(makeGet('http://test/api/registres/laboratoire'));
    const args = prismaMock.laboratoireRapport.findUnique.mock.calls[0]?.[0];
  ...
  it('existing record → returns its counters', async () => {
    prismaMock.laboratoireRapport.findUnique.mockResolvedValue({
      id: 'lr-1',
...
describe('PUT /api/registres/laboratoire', () => {
  ...
  it('negative or non-integer counter → 400 VALIDATION_FAILED', async () => {
    const res = await PUT(makePut({ month: '2026-01', nfsTotal: -1 }));
    expect(res.status).toBe(400);
    expect(prismaMock.laboratoireRapport.upsert).not.toHaveBeenCalled();
  });
  ...
  it('month closed → 409 REGISTER_CLOSED; no upsert', async () => {
    ...
    const res = await PUT(makePut({ month: '2026-01', nfsTotal: 2 }));
  ...
  it('checks closure for the given month, not the current one (timezone-safe)', async () => {
    prismaMock.laboratoireRapport.upsert.mockResolvedValue({
      month: '2025-12',
    } as never);
    await PUT(makePut({ month: '2025-12', nfsTotal: 2 }));
  ...
  it('happy path: upserts only the provided fields, returns full shape', async () => {
    ...
    const res = await PUT(makePut({ month: '2026-01', nfsTotal: 12, nfsPositif: 3 }));

    expect(res.status).toBe(200);
    const upsertArg = prismaMock.laboratoireRapport.upsert.mock.calls[0]?.[0];
    ...
    expect(upsertArg?.update).not.toHaveProperty('anesthesieGenerale');
