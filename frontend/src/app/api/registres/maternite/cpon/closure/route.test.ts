...
describe('GET /api/registres/maternite/cpon/closure', () => {
  it('returns 401 when requireAuth bails', async () => {
    mockRequireOrgMember.mockResolvedValueOnce(
      NextResponse.json({ error: 'Missing token' }, { status: 401 }),
    );
    const res = await GET(makeGet('http://test/api/registres/maternite/cpon/closure'));
  ...
  it('?month= overrides the default', async () => {
    prismaMock.registerClosure.findUnique.mockResolvedValue(null);
    await GET(makeGet('http://test/api/registres/maternite/cpon/closure?month=2025-12'));
    const args = prismaMock.registerClosure.findUnique.mock.calls[0]?.[0];
    expect(args?.where).toEqual({
      organizationId_registerType_month: {
        organizationId: 'org-1',
        registerType: 'maternite-cpon',
        month: '2025-12',
      },
    });
  });
