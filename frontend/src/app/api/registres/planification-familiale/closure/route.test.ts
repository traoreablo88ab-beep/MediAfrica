...
describe('GET /api/registres/planification-familiale/closure', () => {
  it('returns 401 when requireAuth bails', async () => {
    mockRequireOrgMember.mockResolvedValueOnce(
      NextResponse.json({ error: 'Missing token' }, { status: 401 }),
    );
    const res = await GET(makeGet('http://test/api/registres/planification-familiale/closure'));
  ...
  it('?month= overrides the default', async () => {
    prismaMock.registerClosure.findUnique.mockResolvedValue(null);
    await GET(makeGet('http://test/api/registres/planification-familiale/closure?month=2025-12'));
