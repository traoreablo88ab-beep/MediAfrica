...
describe('GET /api/registres/ressources/closure', () => {
  it('returns 401 when requireAuth bails', async () => {
    mockRequireOrgMember.mockResolvedValueOnce(
      NextResponse.json({ error: 'Missing token' }, { status: 401 }),
    );
    const res = await GET(makeGet('http://test/api/registres/ressources/closure'));
  ...
  it('?month= overrides the default, and queries the shared "ressources" registerType', async () => {
    prismaMock.registerClosure.findUnique.mockResolvedValue(null);
    await GET(makeGet('http://test/api/registres/ressources/closure?month=2025-12'));
