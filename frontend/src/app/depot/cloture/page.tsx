...
export default function DepotClotureCaissePage() {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [venteRes, clotureRes] = await Promise.all([
        api<{ ventes: Vente[] }>('/api/depot/ventes'),
        api<{ clotures: Cloture[] }>('/api/depot/cloture'),
      ]);
      setVentes(venteRes.ventes);
      const mine = clotureRes.clotures.find((c) => c.gerantId === user?.id) ?? null;
  ...
  // an ADMIN/OWNER sees every gérant's ventes on /depot for oversight, but
  // their own clôture only ever accounts for their own.
  const mine = ventes.filter((v) => v.gerantId === user?.id && v.statut === 'emise');
  const recetteTheorique = mine.reduce((sum, v) => sum + v.montantTotal, 0);
  const byMode: Record<string, number> = { especes: 0, mobile_money: 0, exoneration: 0 };
  for (const v of mine) byMode[v.modePaiement] = (byMode[v.modePaiement] ?? 0) + v.montantTotal;
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const cloture = await api<Cloture>('/api/depot/cloture', {
        method: 'POST',
        body: { recetteRemise: Number(recetteRemise) },
      });
      setResult({ ...cloture, gerantId: user?.id ?? '', gerantName: user?.name ?? '' });
  ...
  return (
    <main className="min-h-screen bg-[#f9f9f7] md:pl-64">
      ...
      <div className="animate-fade-in-up mx-auto max-w-3xl px-6 py-6">
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-[#0b0b0b]">Clôture de caisse (Dépôt)</h1>
          ...
          <>
            <div className="mb-6 rounded-lg border border-[#e1e0d9] bg-white p-5">
              ...
              <p className="mb-3 text-sm text-[#52514e]">
                {mine.length} vente{mine.length !== 1 ? 's' : ''} émise
