...
const TYPE_LABELS: Record<TypeAlerte, string> = {
  ...
  hors_horaires: 'Activité hors horaires',
  annulations_suspectes: 'Annulations suspectes',
  inactivite: 'Inactivité anormale',
  rupture_sequence: 'Rupture de séquence',
  montant_hors_grille: 'Montant hors grille',
};
...
// Details vary by rule (§ 6.1-6.6 each store different fields) — rendered
// generically as "clé : valeur" rather than special-cased per type, so this
// never drifts from what lib/server/guichet/alertes.ts actually persists.
...
export default function GuichetAlertesPage() {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await api<{ items: Alerte[]; nextCursor: string | null }>(
        `/api/guichet/alertes${buildQuery()}`,
      );
  ...
  async function loadMore() {
    ...
    try {
      const res = await api<{ items: Alerte[]; nextCursor: string | null }>(
        `/api/guichet/alertes${buildQuery(nextCursor)}`,
      );
  ...
  async function markVue(a: Alerte) {
    ...
    setBusyId(a.id);
    try {
      const updated = await api<Alerte>(`/api/guichet/alertes/${a.id}`, {
        method: 'PATCH',
        body: { vue: true },
      });
  ...
  async function saveNote() {
    ...
    setNoteError(null);
    try {
      const updated = await api<Alerte>(`/api/guichet/alertes/${noteTarget.id}`, {
        method: 'PATCH',
      ...
      <div className="animate-fade-in-up mx-auto max-w-5xl px-6 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-[#52514e]">
            Alertes automatiques du Guichet — écarts de caisse, activité hors horaires, annulations
            suspectes, inactivité anormale.
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-sm rounded-lg bg-white p-5 shadow-lg">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Ex: Vérifié avec le guichetier, écart dû à un rendu de monnaie mal compté."
              value={noteText}
