...
const REGISTER_LABEL = 'Registre URENAM';
const REGISTER_TYPE = 'URENAM';
const REGISTER_COLUMN_COUNT = 13;
...
function buildRegisterRows(rows: NutritionRow[]): { headers: string[]; lines: string[][] } {
  ...
  const lines = rows.map((n, i) => {
    ...
    return [
      ...
      n.typeSortie ?? (n.visites.length > 0 ? 'En cours' : ''),
      n.providerName ?? '',
    ];
  });
...
function downloadCsv(month: string, rows: NutritionRow[]): void {
  ...
  a.href = url;
  a.download = `registre-nutrition-urenam-${month}.csv`;
  a.click();
...
function downloadPdf(clinicName: string, month: string, rows: NutritionRow[]): void {
  ...
  downloadRegisterPdf({
    ...
    headers,
    rows: lines,
    fileName: `registre-nutrition-urenam-${month}.pdf`,
  });
}

export default function RegistreNutritionUrenamPage() {
  ...
  const load = useCallback(async (selectedMonth: string) => {
    ...
    try {
      const closureRes = await api<ClosureStatus>(
        `/api/registres/nutrition/urenam/closure?month=${selectedMonth}`,
      );
  ...
  async function onClose() {
    if (
      !window.confirm(
        `Clôturer le registre URENAM de ${month} ? Aucune fiche ne pourra plus être ajoutée ou modifiée pour ce mois.`,
      )
    ) {
      return;
    }
    ...
    setError(null);
    try {
      await api('/api/registres/nutrition/urenam/close', { method: 'POST', body: { month } });
      await load(month);
    } catch (err) {
      setError(friendlyError(err, 'Une erreur est survenue. Réessayez.'));
    } finally {
  ...
  const enCoursCount = items.filter((n) => !n.typeSortie).length;
  const totalVisites = items.reduce((sum, n) => sum + n.visites.length, 0);
      ...
      <div className="animate-fade-in-up mx-auto max-w-7xl px-6 py-6">
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-[#e1e0d9] bg-white p-4 shadow-[0_1px_2px_rgba(11,11,11,0.04)]">
            ...
            <p className="mt-1 text-2xl font-semibold text-[#0b0b0b]">{totalVisites}</p>
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-[#e1e0d9] uppercase tracking-wide text-[#898781]">
                ...
                <th className="px-3 py-2 font-medium">Date admission</th>
                ...
                <th className="px-3 py-2 font-medium">Type de cas</th>
            ...
            <tbody>
              ...
              {items.map((n, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-[#52514e]">{n.typeCas ?? '—'}</td>
                    ...
                    <td className="px-3 py-2">
                      {n.typeSortie ? (
                        <span className="text-[#52514e]">{n.typeSortie}</span>
                      ) : (
              ...
              {!loading && items.length === 0 && !error && (
                <tr>
                  ...
                  >
                    Aucune fiche URENAM ce mois-ci.
                  </td>
                </tr>
              )}
            </tbody>
