...
const REGISTER_COLUMN_COUNT = 10;
...
function buildRegisterRows(rows: VaccinationRow[]): { headers: string[]; lines: string[][] } {
  const headers = [
    'N°',
    'Date',
    'N° dossier',
  ...
  const lines = rows.map((v, i) => [
    String(i + 1),
    formatDate(v.date),
    v.patient.dossierNumber,
    `${v.patient.nom}, ${v.patient.prenom}`,
    String(computeAge(v.patient.dateNaissance)),
    v.patient.sexe,
    v.antigene,
    v.numeroDose != null ? String(v.numeroDose) : '',
    v.numeroLot ?? '',
    v.providerName ?? '',
  ]);
...
function downloadCsv(month: string, rows: VaccinationRow[]): void {
  ...
  a.href = url;
  a.download = `registre-vaccination-${month}.csv`;
  a.click();
  URL.revokeObjectURL(url);
}

function downloadPdf(clinicName: string, month: string, rows: VaccinationRow[]): void {
  const { headers, lines } = buildRegisterRows(rows);
  downloadRegisterPdf({
    title: 'Registre de vaccination (PEV)',
    clinicName,
    month,
    headers,
    rows: lines,
    fileName: `registre-vaccination-${month}.pdf`,
  });
}
...
export default function RegistreVaccinationPage() {
  ...
  const load = useCallback(async (selectedMonth: string) => {
    ...
    try {
      const closureRes = await api<ClosureStatus>(
        `/api/registres/vaccination/closure?month=${selectedMonth}`,
      );
      setClosure(closureRes);

      const { dateFrom, dateTo } = monthBounds(selectedMonth);
      const all: VaccinationRow[] = [];
      let cursor: string | null = null;
      do {
        const params = new URLSearchParams({ dateFrom, dateTo, limit: '50' });
        if (cursor) params.set('cursor', cursor);
        const page: VaccinationPage = await api<VaccinationPage>(
          `/api/vaccination?${params.toString()}`,
        );
        all.push(...page.items);
  ...
  async function onClose() {
    if (
      !window.confirm(
        `Clôturer le registre de ${month} ? Aucune vaccination ne pourra plus être ajoutée pour ce mois.`,
      )
    ) {
      return;
    }
    ...
    setError(null);
    try {
      await api('/api/registres/vaccination/close', { method: 'POST', body: { month } });
      await load(month);
    } catch (err) {
      setError(friendlyError(err, 'Une erreur est survenue. Réessayez.'));
    } finally {
  ...
  const antigeneCounts = new Map<string, number>();
  for (const v of items) {
    antigeneCounts.set(v.antigene, (antigeneCounts.get(v.antigene) ?? 0) + 1);
  }
    ...
    <main className="min-h-screen bg-[#f9f9f7] md:pl-64">
      ...
      <div className="animate-fade-in-up mx-auto max-w-7xl px-6 py-6">
        <div className="mb-6 flex flex-col gap-4 print:hidden sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-[#0b0b0b]">Registre de vaccination (PEV)</h1>
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-[#e1e0d9] bg-white p-4 shadow-[0_1px_2px_rgba(11,11,11,0.04)] sm:col-span-2">
            <p className="text-xs font-medium uppercase tracking-wide text-[#898781]">
              Répartition par antigène
            </p>
            {antigeneCounts.size === 0 ? (
              <p className="mt-1 text-sm text-[#52514e]">Aucune dose ce mois-ci.</p>
            ) : (
              <ul className="mt-2 flex flex-wrap gap-2">
                {[...antigeneCounts.entries()].map(([antigene, count]) => (
                  <li
                    key={antigene}
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-[#e1e0d9] uppercase tracking-wide text-[#898781]">
                ...
                <th className="px-3 py-2 font-medium">Antigène</th>
                <th className="px-3 py-2 font-medium">N° dose</th>
            ...
            <tbody>
              ...
              {items.map((v, i) => (
                ...
                >
                  ...
                  <td className="px-3 py-2 text-[#52514e]">{formatDate(v.date)}</td>
                  <td className="px-3 py-2 text-[#898781]">
                    <Link href={`/patients/${v.patient.id}`} className="hover:underline">
                      {v.patient.dossierNumber}
                    </Link>
                  </td>
                  ...
                  <td className="px-3 py-2 text-[#52514e]">
                    {computeAge(v.patient.dateNaissance)}
                  </td>
                  <td className="px-3 py-2 text-[#52514e]">{v.patient.sexe === 'F' ? 'F' : 'M'}</td>
                  <td className="px-3 py-2 text-[#52514e]">{v.antigene}</td>
                  <td className="px-3 py-2 text-[#52514e]">{v.numeroDose ?? '—'}</td>
                  <td className="px-3 py-2 text-[#52514e]">{v.numeroLot ?? '—'}</td>
                  <td className="px-3 py-2 text-[#52514e]">{v.providerName ?? '—'}</td>
                </tr>
              ))}
