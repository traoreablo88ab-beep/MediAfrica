...
// subscription-billing + 1 Guichet alertes + 1 Dépôt péremption.)
//
...
describe('vercel.json schema (CRON-07, D-20)', () => {
  ...
  it('declares schedules for the 9 canonical crons (Phase 5 + post-audit + multi-tenancy + Guichet + Dépôt)', () => {
    if (!existsSync(VERCEL_JSON)) return;
    const cfg = JSON.parse(readFileSync(VERCEL_JSON, 'utf8')) as VercelConfig;
