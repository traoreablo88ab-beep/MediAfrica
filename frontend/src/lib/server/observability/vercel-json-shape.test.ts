...
// (5 Phase-5 canonical + 1 post-audit email-job-purge + 1 multi-tenancy
// subscription-billing + 1 Guichet alertes + 1 Dépôt péremption + 1 Chariow
...
describe('vercel.json schema (CRON-07, D-20)', () => {
  ...
  it('declares schedules for the 10 canonical crons (Phase 5 + post-audit + multi-tenancy + Guichet + Dépôt + Chariow)', () => {
    if (!existsSync(VERCEL_JSON)) return;
    const cfg = JSON.parse(readFileSync(VERCEL_JSON, 'utf8')) as VercelConfig;
    const paths = (cfg.crons ?? []).map((c) => c.path).sort();
    expect(paths).toEqual([
      '/api/cron/depot-peremption',
      '/api/cron/email-job-purge',
      '/api/cron/email-queue-drain',
      '/api/cron/guichet-alertes',
